...
int yuv422ToGray(byte **rgb, byte **gray, int buffer_size) {
    // Take size for gray image and allocate memory
    int gray_size = buffer_size / 2;
...
int rgbaToGray(byte **rgb, byte **gray, int buffer_size)
{
    // Take size for gray image and allocate memory
    int gray_size = buffer_size / 4;
    *gray = malloc(sizeof(byte) * gray_size);
    // Make pointers for iteration
    byte *p_rgb = *rgb;
    byte *p_gray = *gray;
...
int sobelFilter(CALLBACK callback,byte **rgb, byte **gray, 
                byte **sobel_h_res, byte **sobel_v_res, byte **contour_img, 
    ...
    int gray_size = callback(rgb, gray, rgb_size);
    printf("[%s],%d,%d,%d,%d,%d\r\n",__func__,rgb_size,width,height, channels,gray_size);
